{{
  config(
    materialized='table',
    tags=['silver', 'cleaned', 'fact', 'lineitem']
  )
}}
...
select
    ...
    l.l_extendedprice / nullif(l.l_quantity, 0) as unit_price,
    ...
    extract(month from l.l_shipdate) as ship_month,
    
    -- lead time calculations
    datediff('day', o.order_date, l.l_shipdate) as order_to_ship_days,
    datediff('day', l.l_commitdate, l.l_shipdate) as commit_to_ship_days,
    datediff('day', l.l_shipdate, l.l_receiptdate) as ship_to_receipt_days,
    datediff('day', o.order_date, l.l_receiptdate) as total_lead_time_days,
    ...
    case 
        when datediff('day', l.l_commitdate, l.l_shipdate) <= 0 then 'ON_TIME'
        when datediff('day', l.l_commitdate, l.l_shipdate) <= 7 then 'SLIGHTLY_LATE'
        else 'VERY_LATE'
